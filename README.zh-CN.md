...
- 展开后可以再次隐藏同一条帖子。
- 折叠帖子时会连同图片和视频一起隐藏。
- 设置页在独立标签页中打开，可配置 API Key、Jev API 地址、语言、兴趣主题、价值偏好、类别过滤和分数阈值。
...
API Key 只保存在当前设备，不会同步到 Google 账户。分类请求会发送到设置中配置的 Jev API 地址，默认为 `https://api.typesafe.ai/v1/systemone`，清空该输入框即可恢复默认。扩展会在安装时申请 X/Twitter 页面和默认 TypeSafe 主机的访问权限；只有在你保存自定义地址时，才会额外申请该主机的权限。
